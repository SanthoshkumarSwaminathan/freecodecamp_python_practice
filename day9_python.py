...
# Enclosing scoper(E) : Variables defined in enclosing or nested functions.
# Global scope(G): Variables defined at the top level of the module or file.
# Build-in scope(B): Reserved names in python for predefined functions, moudles, keywords, and objects.
...
# Enclosing scope means that a function that's nested inside another function can access the variables of the fucntion it's nested within.
...
# One solution is to initialize res as an empty string in the enclosing scope, which is within outer_func. Then within inner_fucn, make res a non-local variable with the nonlocal keyword:
...
def outer_func():
    ...
    def inner_func():
        nonlocal res # Allow modification of an enclosing
        res = 'How are you?'
